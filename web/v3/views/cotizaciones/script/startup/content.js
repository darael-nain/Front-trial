...
var params = {
	entity: 'Cotizacion',
	// buttons: [ 'save', 'delete', 'clone_current', 'convert_negocio', 'discard', 'restore', 'exit', 'preview', 'share', 'template', 'validate', 'offline_mode', 'request_convert_negocio' ],
	buttons: ['save', 'refresh', 'convert_negocio', 'delete', 'clone_current', 'conversion_negocio_request', 'discard', 'restore', 'exit', 'preview', 'share', 'template', 'validate', 'offline_mode', 'share_cot_val', 'ultimate_val'],
	data: function () {

		//REAPER ----> este es el metodo de guardado en cotizacion



		//Para el correcto funcionamiento del tipo cambio al guardar
		...
		let fields = {
			...
			'cotizacion[total][item][adicional]': unaBase.utilities.transformNumber(document.getElementsByName('cotizacion[total][item][adicional]')[0].value),
			...
			'cotizacion[condiciones][fecha_proyecto]': document.querySelector('input[name="cotizacion[condiciones][fecha_proyecto]"]')?.value || '',
			...
			//'cotizacion[moneda_impresion]': ($('select[name="cotizacion[moneda]"]')[0].options[ $('select[name="cotizacion[moneda]"]')[0].options.selectedIndex]).value,
		...
		// fields['cotizacion[tipo_cambio]'] = $('input[name="cotizacion[tipo_cambio]"]').length > 0 ? ($('input[name="cotizacion[tipo_cambio]"]').val()).toString().replace(/\./g, '') : 0;
		//fields['cotizacion[tipo_cambio_trabajo]'] = $('input[name="cotizacion[moneda_impresion]"]').length > 0 ? $('input[name="cotizacion[moneda_impresion]"]').val().replaceAll(".", "") : 0;
		...
		let success = true;
		//REAPER toneg --OK
		$('section.sheet section.sobrecargos   li').each(function () {
			...
			$.ajax({
				...
				success: function (data) {
					...
					if (data.success) {
						console.log('-------------------- Sobrecargo guardado -------------')

					} else {

						success = false;
						console.warn('--------------------  Sobrecargo no se pudo guardar ----------   ' + error.statusText ? error.statusText : "")
	...
	validate: function () {
		...
		if (contacto_asociado_obligatorio) {
			$('input[name="cotizacion[empresa][contacto][id]"]').each(function (key) {
				if (parseFloat($(this).attr('data-id')) == 0 || $(this).val() == "") {
		...
		if (!isValidContacto) {
			msgError += "- Falta seleccionar contacto asociado.<br/>";
			isValidEmpresa = false;
		}
		...
		$('table.items > tbody > tr > * > [name="item[][nombre]"] tr:not(.title):not(.itemParent)').each(function (key, item) {
			if ($(item).val() == '') {
				isValidItems = false;
				$(item).invalid();
...
$(document).ready(function () {
	...
	//  cantidad parametrizable en cotización  simon02agosto2017
	// $('select[name=cantidad_item_negocio]').val(cantidad_item_negocio);
	...
	if (typeof convertir_en_negocio == 'undefined') {
		...
		if (parseFloat($('#main-container').data('index')) > 0) {

			// --ini-- agregado para verificar que cot este validado antes de mostrar el boton "solicitar conversion", g, 12.10.18
			if ($('section.sheet').find('span.validation-status').text() == 'Validada') {
				$('[data-name="conversion_negocio_request"]').show();
			}
			// --fin-- agregado para verificar que cot este validado antes de mostrar el boton "solicitar conversion", g, 12.10.18
	...
	$('button.unlock.empresa').button({
		...
		// target.find('input[name="cotizacion[empresa][rut]"]').data("checkrut", false);
		try {
			if (target.find('input[name="cotizacion[empresa][rut]"]')[0]) {
				target.find('input[name="cotizacion[empresa][rut]"]')[0].dataset.checkrut = false
				target.find('input[name="cotizacion[empresa][rut]"]')[0].dataset.rut = target.find('input[name="cotizacion[empresa][rut]"]').val();
			}
		...
		// target.find('input[name="cotizacion[empresa][rut]"]').data("rut", target.find('input[name="cotizacion[empresa][rut]"]').val());
	...
	$('button.fetch.exchange-rate').button({
		...
		// // sistema nuevo
		// if (target.find('input[name="cotizacion[tipo_cambio]"]').length > 0)
		// 	$.ajax({
		...
		// 		dataType: 'json',
		// 		success: function(data) {
		// 			target.find('input[name="cotizacion[tipo_cambio]"]').val(parseFloat(data.rate).toFixed(8).replace(/\./g, ',')).trigger('change');
		// 		}
		// 	});
		// // sistema antiguo
		// else
	...
	$('button.edit.save.contacto').click(function (event) {
		...
		$.ajax({
			url: '/4DACTION/_V3_setContactoByEmpresa19',
			dataType: 'json',
			data: fields,
			async: false,
			success: function (data) {
				if (data.success) {
					...
					$('input[name="cotizacion[empresa][contacto][id]"]').attr('data-id', data.id);
	...
	unaBase.toolbox.form.autocomplete({
		...
		select: function (event, ui) {
			...
			$('input[name="cotizacion[empresa][contacto][id]"]').attr('data-id', ui.item.id);
			...
			var old_empresa = $('input[name="cotizacion[empresa][contacto][id]"]').attr('data-id');
	...
	if ($('input[name="cotizacion[proyecto]"]').length > 0)
		$('input[name="cotizacion[proyecto]"]').autocomplete({
			...
			select: function (event, ui) {
				...
				$('input[name="cotizacion[proyecto]"]').attr('data-id', ui.item.id);
	...
	if ($('section.sheet').data('index') == -1)
		htmlObject = $(`
			<ul id="dropdown_menu" class="dropdown-menu" style="position: absolute; top: 49px; right: 13px; z-index: 1000;left:80%; "> 
		...
		htmlObject = $(`
			<ul id="dropdown_menu" class="dropdown-menu" style="position: absolute; top: 49px; right: 13px; z-index: 1000;left:80%;"> 
				...
				<li><a href="/v3/views/cotizaciones/dialog/xml_load.shtml" data-size="small"><span class="ui-icon ui-icon-calculator"></span>Cargar xml</a></li>
				<li><a href="/v3/views/cotizaciones/dialog/import_cot_csv.shtml" data-size="small"><span class="ui-icon ui-icon-calculator"></span>Cargar ítems desde CSV</a></li>
